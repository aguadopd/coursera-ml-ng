...
Si usamos el error cuadrático medio como función de costo para optimizar con el descenso por el gradiente, vamos a tener que derivar una función no lineal. Esto es porque la función logística/sigmoidea $h(z)$ no es lineal con respecto a los parámetros \theta, y por tanto el error cuadrático medio no es una función convexa; esto implica que tiene (¿o puede tener?) más de un mínimo.

Lo que hacemos entonces es proponer otra función de costo que sea convexa y diferenciable. Por supuesto, tiene que penalizar las predicciones/hipótesis erróneas. La que se propone es

\[ J(h_\theta(x)) = J(h(\theta,x) = \quad -log(h_\theta(x)) \quad \text{si} y = 1   \]
\[ J(h_\theta(x)) = J(h(\theta,x) = \quad -log(1-h_\theta(x)) \quad \text{si} y = 0   \]
...
#+begin_src octave :exports none
  ...
  title('J(h( \theta ,x) = -log(h_\theta(x))')
  legend off;
  % l = legend;
...
#+begin_src octave :exports none
  ...
  set(gca, "linewidth", 4, "fontsize", 18)
  title('J(h( \theta ,x) = -log(1-h_\theta(x))')
  legend off;
  % l = legend;
  % set(l, "fontsize", 14, "location","east")
